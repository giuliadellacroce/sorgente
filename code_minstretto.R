    ...
    if(stoc==1){ #Stoc. method
      ...
      for(iter1 in 1:N)
      {
        for(iter2 in 1:M)
        {
          ...
          b_sup=min(I_tot_t[1]-1,i_corr[2]-y_corr[2]+diff_R[2]-1)
          ...
          for(t in 2:(n-1)){
            ...
            bound_inf=max(z[t],i_corr[t]-i_corr[t-1],
                          diff_R[t]+i_corr[t]-I_tot_t[t-1]+1)
            bound_sup=min(Y_tot_t[t]-n_t[t]+z[t],
                          diff_R[t]+i_corr[t]-i_corr[t-1]-1,i_corr[t]-1)
            ...
            if(t==(n-1)){ #Last i (time=n-1)
              bound_inf=max(i_corr[t-1]+y_corr[t]-diff_R[t]+1,y_corr[t]+1)
              bound_sup=min(I_tot_t[t]-1,i_corr[t-1]+y_corr[t],
                            I_tot_t[t-1]-diff_R[t]+y_corr[t]-1)
              ...
              bound_sup=min(I_tot_t[t]-1,i_corr[t-1]+y_corr[t],
                            I_tot_t[t-1]-diff_R[t]+y_corr[t]-1,
                            i_corr[t+1]-y_corr[t+1]+diff_R[t+1]-1)
              prop=proposta_i(t,Nrho[t],bound_inf,bound_sup,i_corr)
              i_prop=prop[1]
              proposta_num=prop[2]
              proposta_den=prop[3]
