#USATI (POI DA TOGLIERE):
# Nn=c(rep(10,5),rep(10,4),rep(10,7),rep(20,3),rep(40,10),rep(70,5),rep(100,10),rep(150,n-44))
# Nrho=c(rep(20,5),rep(20,4),rep(20,7),rep(20,3),rep(50,10),rep(130,11),rep(150,n-40))
# N_eps=5

sorgente <- function(stoc,kstart,epsstart,stepk,stepeps,x_axis,num_it,Burnin,Thin,Nn,Nrho,N_eps) { 
  ...
  #kstart = starting value of parameter k to maximize the likelihood (multiple starting values should be tried)
  #epsstart = same as before for parameter I0 (called also epsilon) (multiple starting values should be tried)
  #stepk = starting gridwith to compute profile C.I. for k
  #stepeps = same for I0
  #x_axis = vector with the days in which for the present dataset n_t!=0
  #num_it = Total number of iterations of the MCMC (for stoc. method)
  #Burnin (for stoc. method)
  #Thin (for stoc. method)
  ...
  if(stoc==0){ #Det. method
    ...
    #in the first row -> estimate of k, 95% profile C.I., 95% C.I. from parametric bootstrap
    #in the second row -> same for parameter I0 
    ...
    Griglia=seq(est@coef[1]-1,est@coef[1]+1,stepk)
      ...
      for(l in 1:4){
        for(r in 1:(length(profsx-1))){
          if((profsx[r]>=q1)&(profsx[r+1]<=q1)){
            grigliasx=seq(grigliasx[r],grigliasx[r+1],stepk*10^(-l))
          }
          if((profdx[r]<=q1)&(profdx[r+1]>=q1)){
            grigliadx=seq(grigliadx[r],grigliadx[r+1],stepk*10^(-l))
          }
        }
        profsx=PLk(grigliasx)
        profdx=PLk(grigliadx)
        if(l==4){
          pos=min(abs(profsx[r]-q1),abs(profsx[r+1]-q1))
          CIinf=grigliasx[pos]
          pos=min(abs(profdx[r]-q1),abs(profdx[r+1]-q1))
          CIsup=grigliadx[pos]
        }
      }
    ...
    PLeps <- function(griglia) { 
      lg=length(griglia)
      profilo=numeric(lg)
      for(i in 1:lg){
        profilo[i]<-optimize(leps,interval=c(1,10),epsilon=griglia[i])$objective  
      }
      return(2*(profilo-l(est@coef[1],est@coef[2])))
    }
    
    Griglia=seq(max(0.1,est@coef[2]-30),est@coef[2]+30,stepeps)
    
    Prof_eps <- function(griglia) { 
      grigliasx=griglia
      grigliadx=griglia
      profsx=PLeps(grigliasx)
      profdx=PLeps(grigliadx)
      for(l in 1:4){
        for(r in 1:(length(profsx-1))){
          if((profsx[r]>=q1)&(profsx[r+1]<=q1)){
            grigliasx=seq(grigliasx[r],grigliasx[r+1],stepeps*10^(-l))
          }
          if((profdx[r]<=q1)&(profdx[r+1]>=q1)){
            grigliadx=seq(grigliadx[r],grigliadx[r+1],stepeps*10^(-l))
          }
        }
        profsx=PLeps(grigliasx)
        profdx=PLeps(grigliadx)
    ...
    sol[2,2:3]=Prof_eps(Griglia)
    ###################
    
    ############ Parametric Bootstrap
    kstart_pb=est@coef[1]-0.5
    epsstart_pb=est@coef[2]+5
    #this choice for the starting values in the P.B. is to avoid going in regions where the likelihood is not finite
    k=est@coef[1]
    eps=est@coef[2]
