...
sorgente <- function(stoc,kstart,epsstart,stepk,stepeps,maxk,x_axis,incr_k,incr_eps,num_it,Burnin,Thin,Nn,Nrho,N_eps,kstart_MCMC,epsstart_MCMC) { 
  ...
  #stepeps = same for I0
  #max_k = maximum value imposed to parameter k in the computation of the profile likelihood C.I. of I0
  #x_axis = vector with the days in which for the present dataset n_t!=0
  #incr_k = to help define the starting point for parameter k in the  parametric bootstrap
  #incr_eps = same for parameter I0
  #num_it = Total number of iterations of the MCMC (for stoc. method)
  #Burnin (for stoc. method)
  #Thin (for stoc. method)
  #Nn = vector of increments used in the uniform proposals (MH) of the Y_j terms, to be considered together with bounds
  #Nrho = same for the I_j terms
  #N_eps = same for I0
  #kstart_MCMC = starting value for parameter k in the MCMC
  #epsstart_MCMC = same for parameter I0
  ...
  if(stoc==0){ #Det. method
    ...
    if(stoc==1){ #Stoc. method
      ...
      #Columns from 2 to n+1 contain the samples of Y_j 
      #(we are interested in columns 3:n+1)
      #Columns from n+2 to 2n+1 contain the samples of I_j, in particular column n+2 contains parameter I0. Column 2n+1 contains I_n, which we are not interested in
    }
  }
  return(sol)
}
